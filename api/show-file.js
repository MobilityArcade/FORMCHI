export default async function handler(req,res){
  ...
  try{
    ...
    if(!r.ok){console.error('AKI 7.1.6 file API error',r.status,raw.slice(0,1200));return res.status(r.status).json({ok:false,error:'File analysis failed'});}
  ...
  }catch(err){console.error('AKI 7.1.6 file server error',err);return res.status(500).json({ok:false,error:'Unable to analyze file'});}
}

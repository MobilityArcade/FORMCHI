export default async function handler(req,res){
  ...
  try{
    ...
    const raw=await r.text();if(!r.ok){console.error('AKI 10.5.4 handoff API error',r.status,raw.slice(0,800));return res.status(r.status).json({ok:false,error:'Handoff resolution failed'})}
  ...
  }catch(err){console.error('AKI 10.5.4 handoff server error',err);return res.status(500).json({ok:false,error:'Unable to create handoff'})}
}
